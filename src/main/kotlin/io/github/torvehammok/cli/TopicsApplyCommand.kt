...
@Command(
    name = "topics-apply",
    description = ["Applies Kafka topic changes based on the configuration."],
    mixinStandardHelpOptions = true
)
class TopicsApplyCommand : Runnable {
    ...
    private fun apply(ctx: Ctx) {
        val topicService = ctx.get(TopicService::class.java)
        topicService.applyTopicChanges()
    }
}
